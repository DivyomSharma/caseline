...
export default function CasesClient({ initialCases, stations, officers }: CaseListProps) {
  ...
  const getStatusBadge = (s: string) => {
    ...
    switch (s) {
      ...
      default:
        return <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-slate-100 text-slate-700 border border-slate-200">{label}</span>;
    }
  };
  ...
  const getPriorityBadge = (p: string) => {
    switch (p) {
      case 'low':
        return <span className="inline-flex items-center px-1.5 py-0.2 rounded text-[9px] font-bold bg-slate-50 text-slate-600 border border-slate-100">Low</span>;
      case 'medium':
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
        <div>
          <h1 className="text-xl font-extrabold text-slate-900 tracking-tight">Cases Directory</h1>
        ...
        <Link
          href="/fir/new"
          className="inline-flex items-center space-x-2 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs px-4 py-2.5 rounded-lg shadow-sm self-start transition-all"
        >
        ...
        <div className="flex flex-col md:flex-row md:items-center space-y-3 md:space-y-0 md:space-x-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-lg pl-9 pr-4 py-2.5 text-xs text-slate-800 placeholder:text-slate-400 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
            />
          </div>
          {(search || status || priority || stationId || officerId) && (
            <button
              onClick={handleResetFilters}
              className="text-xs text-slate-500 hover:text-slate-800 font-bold px-3 py-2 border border-slate-200 rounded-lg hover:bg-slate-50"
            >
              Reset Filters
            </button>
          )}
              ...
              {paginatedCases.length === 0 ? (
                ...
                paginatedCases.map((c) => (
                  <tr key={c.id} className="hover:bg-slate-50/60 transition-all">
                    <td className="py-3.5 px-4 font-bold text-slate-800 tracking-tight font-mono">
                      {c.case_number}
                    </td>
          ...
          <div className="bg-slate-50/50 border-t border-slate-100 p-4 flex items-center justify-between">
            ...
            <div className="flex space-x-1.5">
              <button
                ...
                className="px-3 py-1.5 text-[10px] font-bold text-slate-600 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed select-none"
              >
                Previous
              </button>
              <button
