  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-xl font-extrabold text-stone-900 tracking-tight">Criminal Records Index</h1>
      ...
      <div className="bg-white border border-stone-200 rounded-xl p-4 shadow-sm flex items-center">
        <form className="relative flex-1" method="GET" action="/criminals">
          ...
          <input
            ...
            className="w-full bg-stone-50 border border-stone-200 rounded-lg pl-9 pr-4 py-2.5 text-xs text-stone-800 placeholder:text-stone-400 focus:outline-none focus:border-amber-500"
          />
        </form>
      </div>
        ...
        {criminals.length === 0 ? (
          ...
          criminals.map((c: any) => (
            <div key={c.id} className="bg-white border border-stone-200 rounded-xl p-4 shadow-sm flex flex-col justify-between space-y-4">
              ...
              <div className="text-[10px] text-stone-555 font-semibold space-y-1">
              ...
              <Link
                href={`/criminals/${c.id}`}
                className="w-full text-center inline-flex items-center justify-center space-x-1.5 border border-stone-200 hover:border-stone-350 bg-stone-50/20 hover:bg-stone-50 text-stone-700 font-bold px-3 py-2 rounded-lg text-xs shadow-sm transition-all"
              >
