...
export default function SettingsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Header */}
      <div>
        <h1 className="text-xl font-extrabold text-stone-900 tracking-tight">System Settings</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Profile Card */}
        <div className="bg-white border border-stone-200 p-5 rounded-xl shadow-sm space-y-4 h-fit">
          <h3 className="text-xs font-bold text-stone-800 uppercase tracking-widest border-b border-stone-100 pb-3 flex items-center space-x-2">
          ...
          {user ? (
            <div className="space-y-3.5 text-xs font-semibold">
              <div>
                <span className="text-[10px] text-stone-400 block uppercase font-bold">Log Account Owner</span>
                <span className="text-stone-800 font-extrabold">{user.full_name}</span>
              </div>
              <div>
                <span className="text-[10px] text-stone-400 block uppercase font-bold">Registrar Email</span>
                <span className="text-stone-800">{user.email}</span>
              </div>
              <div>
                <span className="text-[10px] text-stone-400 block uppercase font-bold">Active Authorization role</span>
                <span className="text-amber-600 font-black uppercase tracking-wider">{user.role}</span>
              </div>
            </div>
          ) : (
            <div className="text-xs text-stone-400">Loading user profile...</div>
          )}
        </div>

        {/* Database Adapter Configuration */}
        <div className="bg-white border border-stone-200 p-5 rounded-xl shadow-sm space-y-4 lg:col-span-2">
          <h3 className="text-xs font-bold text-stone-800 uppercase tracking-widest border-b border-stone-100 pb-3 flex items-center space-x-2">
          ...
          <div className="space-y-4">
            ...
            <div className="p-4 rounded-xl border border-stone-200 bg-stone-50/50 space-y-3">
              <div className="text-xs font-bold text-stone-700">Database Configuration</div>
            ...
            <div className={`p-4 rounded-lg border text-xs ${
              ...
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="mt-1 leading-5 font-medium text-stone-600">
                    {isSupabase 
            ...
            {/* Env Table */}
            <div className="border border-stone-150 rounded-lg overflow-hidden text-xs">
              <div className="bg-stone-50 border-b border-stone-200 py-2 px-3 font-bold text-[10px] text-stone-400 uppercase tracking-wider">
                System Credentials Check
              </div>
              <div className="divide-y divide-stone-100 p-3 space-y-2">
                <div className="flex justify-between items-center">
                  <span className="font-bold text-stone-600 font-mono">NEXT_PUBLIC_SUPABASE_URL</span>
                  {isSupabase ? (
                    ...
                    <span className="text-[10px] bg-stone-100 text-stone-500 px-2 py-0.5 rounded-full font-bold">Unset (Local DB)</span>
                  )}
                </div>

                <div className="flex justify-between items-center pt-2">
                  <span className="font-bold text-stone-600 font-mono">NEXT_PUBLIC_SUPABASE_ANON_KEY</span>
                  {isSupabase ? (
                    <span className="text-[10px] bg-emerald-100 text-emerald-800 px-2 py-0.5 rounded-full font-bold flex items-center space-x-1">
                      <Check className="w-3 h-3" />
                      <span>Configured</span>
