...
export default function AnalyticsClient({ stats }: AnalyticsProps) {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Header */}
      <div>
        <h1 className="text-xl font-extrabold text-slate-900 tracking-tight">Statistical Crime Analytics</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={idx} className="bg-white border border-slate-200 p-4.5 rounded-xl shadow-sm">
            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block">{item.label}</span>
            <span className="text-xl font-black text-slate-800 tracking-tight mt-1.5 block">{item.value}</span>
            <span className="text-[9px] text-slate-500 block mt-1.5 font-medium">{item.desc}</span>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white border border-slate-200 p-5 rounded-xl shadow-sm flex flex-col h-[380px]">
          <h3 className="text-xs font-bold text-slate-800 uppercase tracking-widest mb-4 flex items-center space-x-1.5">
