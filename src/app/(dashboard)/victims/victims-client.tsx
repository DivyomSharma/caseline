  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-xl font-extrabold text-[var(--color-ink)] tracking-tight">Complainants & Victims Registry</h1>
      ...
      <div className="bg-white border border-[var(--color-lavender-border)] rounded-2xl p-4 shadow-sm flex items-center">
        <form className="relative flex-1" method="GET" action="/victims">
          <Search className="absolute left-3 top-3.5 w-4 h-4 text-[var(--color-ink-soft)]/70" />
          <input
            type="text"
            name="search"
            defaultValue={search || ''}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-[var(--color-lavender-border)] text-xs">
              {victims.length === 0 ? (
                <tr>
                  <td colSpan={5} className="py-8 text-center text-[var(--color-ink-soft)]/70 font-medium">
                    No victim records found. Register complainants via this directory or register new FIRs.
                  </td>
                </tr>
              ) : (
                victims.map((v: any) => (
                  <tr key={v.id} className="hover:bg-[var(--color-lavender)]/60 transition-all">
                    ...
                    <td className="py-3.5 px-4 text-[var(--color-ink-soft)] font-semibold font-mono">
                      {isViewer ? (
                        <span className="inline-flex items-center space-x-1 text-[var(--color-ink-soft)]/70 font-normal italic">
                          <Lock className="w-3 h-3 text-[var(--color-ink-soft)]/50 shrink-0" />
                    ...
                    <td className="py-3.5 px-4 text-[var(--color-ink-soft)] font-medium">
                      {isViewer ? (
                        ...
                        <span className="flex items-center space-x-1.5">
                          <MapPin className="w-3.5 h-3.5 text-[var(--color-ink-soft)]/70 shrink-0" />
                    ...
                    <td className="py-3.5 px-4 text-[var(--color-ink-soft)] max-w-xs truncate font-medium">
                      {v.notes}
                    </td>
                  </tr>
                ))
