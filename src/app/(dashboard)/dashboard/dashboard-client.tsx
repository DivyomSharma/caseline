...
export default function DashboardClient({ stats }: DashboardClientProps) {
  const kpis = [
    ...
    { label: "Active Officers", value: stats.activeOfficers, icon: UserCheck, color: "text-stone-700 bg-stone-100 border-stone-200" }
  ];
  ...
  const quickActions = [
    { label: "Register FIR", desc: "Log a new complaint & init case", href: "/fir/new", icon: FileText, color: "hover:border-amber-200 hover:bg-amber-50/20" },
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Welcome Title */}
      <div>
        <h1 className="text-2xl font-extrabold text-stone-900 tracking-tight">Command Center</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-6 gap-4">
        {kpis.map((kpi) => {
          const Icon = kpi.icon;
          return (
            <div key={kpi.label} className="bg-white border border-stone-200 p-4 rounded-xl shadow-sm flex flex-col justify-between">
          ...
          {quickActions.map((act) => {
            ...
            return (
              <Link 
                key={act.label} 
                href={act.href}
                className={`border border-stone-100 p-3.5 rounded-lg flex flex-col items-start transition-all cursor-pointer ${act.color}`}
              >
                <div className="w-8 h-8 bg-stone-50 border border-stone-100 rounded-lg flex items-center justify-center mb-2.5">
                  <Icon className="w-4.5 h-4.5 text-stone-800" />
                </div>
                <div className="text-xs font-bold text-stone-900 leading-4">{act.label}</div>
                <div className="text-[9px] text-stone-400 mt-0.5 leading-3">{act.desc}</div>
              </Link>
            );
          })}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Crime Type Chart */}
        <div className="bg-white border border-stone-200 p-5 rounded-xl shadow-sm flex flex-col h-[350px]">
          <h3 className="text-xs font-bold text-stone-800 uppercase tracking-widest mb-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Activity Stream */}
        <div className="bg-white border border-stone-200 p-5 rounded-xl shadow-sm lg:col-span-2">
          <h3 className="text-xs font-bold text-stone-800 uppercase tracking-widest mb-4 flex items-center justify-between">
            <span>Recent Activity Audit Trail</span>
            <span className="text-[10px] text-stone-400 normal-case font-medium">Real-time status updates</span>
          </h3>
                  ...
                  <li key={activity.id}>
                    <div className="relative pb-8">
                      ...
                      <div className="relative flex space-x-3">
                        <div>
                          <span className="h-8 w-8 rounded-lg bg-stone-50 border border-stone-100 flex items-center justify-center">
                        ...
                        <div className="flex-1 min-w-0 pt-1.5 flex justify-between space-x-4">
                          <div>
                            <p className="text-xs text-stone-800 font-semibold">
                              {activity.title} <span className="font-normal text-stone-500">for</span>{' '}
                              <Link href={`/cases/${activity.case_id}`} className="font-bold text-amber-600 hover:text-amber-800 hover:underline">
                                {activity.case_number}
                              </Link>
                            </p>
                            <p className="text-[10px] text-stone-400 mt-0.5 leading-3">
                              {activity.description}
                            </p>
                          </div>
                          <div className="text-right text-[10px] whitespace-nowrap text-stone-400 font-medium">
                            <time dateTime={activity.created_at}>{formatTimeAgo(activity.created_at)}</time>
                            <div className="text-[8px] font-bold text-stone-500 mt-1">{activity.full_name}</div>
                          </div>
                        </div>
                      </div>
                    </div>
        ...
        <div className="bg-white border border-stone-200 p-5 rounded-xl shadow-sm h-fit">
          <h3 className="text-xs font-bold text-stone-800 uppercase tracking-widest mb-4">
          ...
          <div className="space-y-3">
            {stats.charts.casesByStation.map((st: any) => {
              ...
              return (
                <div key={fullStation.id} className="p-3 rounded-lg border border-stone-100 bg-stone-50/30 flex items-start space-x-3">
                  <div className="w-8 h-8 rounded bg-stone-100 flex items-center justify-center shrink-0 border border-stone-200">
                    <MapPin className="w-4 h-4 text-stone-500" />
                  </div>
                  <div className="min-w-0">
                    <div className="text-xs font-bold text-stone-800 truncate">{fullStation.name}</div>
                    <div className="text-[9px] text-stone-500 font-semibold">{fullStation.contact}</div>
                    <div className="text-[8px] font-bold text-stone-400 mt-0.5 truncate uppercase">
