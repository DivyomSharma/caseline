...
export default function AIChatWidget() {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-50">
      {open && (
        <div className="mb-3 w-80 h-96 bg-white border border-stone-200 rounded-xl shadow-xl flex flex-col overflow-hidden">
          <div className="flex items-center justify-between px-3 py-2 border-b border-stone-100 bg-stone-900 text-white">
          ...
          <div className="border-t border-stone-100 p-2 flex items-center space-x-2">
            <input
              value={input}
              onChange={(e) => setInput(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && sendMessage()}
              placeholder="Ask the AI assistant..."
              className="flex-1 text-xs px-2.5 py-1.5 border border-stone-200 rounded-lg outline-none focus:border-stone-400"
              disabled={loading}
            />
            <button
              onClick={sendMessage}
              disabled={loading || !input.trim()}
              className="p-1.5 rounded-lg bg-stone-900 text-white disabled:opacity-40"
            >
